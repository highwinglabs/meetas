...
export default function RecordingPanel({ consentOk, onNeedConsent }: { consentOk: boolean; onNeedConsent: () => void }) {
  ...
  const stop = async () => { if (!active) return; setBusy(true); setError(null); try { await api.stop(active.id); setNotice(t("rec.stopped")); setActive(null); } catch (e) { setError((e as Error).message); } finally { setBusy(false); } };
  ...
  return <div className="panel">
    <h1>{active ? active.title : t("rec.new_meeting")}</h1>{error && <Note kind="error">{error}</Note>}{notice && <Note kind="ok">{notice}</Note>}
