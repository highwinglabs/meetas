      ...
      {!tasks.length ? <Note>{t("task.empty")}</Note> : <div className="task-list">
        {tasks.map((task) => <div key={task.id} className={"task " + (task.display_status ?? task.status)}>
          <div className="task-main">
            ...
            <div className="task-sub"><span className="chip">{task.source ?? t("task.source_manual")}</span>{task.source_segment_id ? <span className="dim">{t("task.from_transcript")}</span> : <span className="dim">{t("task.manual_created")}</span>}</div>
